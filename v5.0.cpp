...
void inserirParticipante() {
    ...
    do {
        std::cout << "ID: ";
        std::cin >> novoParticipante->id;
        if (idJaExiste(novoParticipante->id)) {
            std::cout << "ID já existe. Tente novamente.\n";
        }
    } while (idJaExiste(novoParticipante->id));
...
// Função para liberar a memória alocada para os participantes
...
// Função para liberar a memória alocada para as contribuições
void limparContribuicoes() {
